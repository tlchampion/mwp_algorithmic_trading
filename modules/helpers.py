...
# set default variables for stock start dates
default_start_year = 2017
default_start_month = 12
default_start_day = 31
...
# list of names for all available portfolios
portfolio_options = ['conservative', 'balanced', 'growth', 'aggressive', 'alternative']
...
# obtain weights based upon portfolio name

def get_weights_by_name(name):
...
# get tickers based on portoflio name
def get_ticker_by_port_name(name):
    if (name == "conservative"):
        return list(conservative.keys())
    elif (name == "balanced"):
        return list(balanced.keys())
...
def get_stocks(ticker_list, year=default_start_year, month=default_start_month, day=default_start_day):
